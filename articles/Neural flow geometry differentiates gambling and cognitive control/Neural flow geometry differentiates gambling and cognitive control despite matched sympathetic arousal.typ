...
Of 135 possible sessions (45 subjects × 3 tasks), 134 produced valid epoch-level data (one GG session failed; see below), yielding 57,526 epochs. Task coverage by modality was uniformly excellent, as summarized in @tab-qc:
...
This null is important as a background for the MNJ results: the contrast between global-coordinate insensitivity and flow-geometry sensitivity, illustrated directly in @fig-mnps-mnj, is the central finding of the paper.
...
Block-level Jacobian estimates (from the `block_frobenius_mean` and `c_rot_mean` outputs of the NMD summarize step) replicated these patterns: Friedman $chi^2 = 47.77$, $p < 0.0001$ for Frobenius; $chi^2 = 34.41$, $p < 0.0001$ for rotational component. GG vs CC block Frobenius: $d = 1.70$, $p < 10^{-12}$. @fig-mnj-effects summarizes effect sizes for all six MNJ metrics across the three pairwise task contrasts.
...
LOOSO for the four primary MNJ metrics (Frobenius, spectral radius, rotation norm, rotational power) showed 100% sign consistency across all 44 LOO iterations, with 100% of iterations reaching $p < 0.05$. Bootstrap confidence intervals from 2000 resamples are given in @tab-bootstrap and visualized alongside the LOOSO range in @fig-bootstrap:
...
Spearman correlations between the GG–CC difference in Frobenius and candidate confound differences (subject medians, $N = 44$ intersection sample) are reported in @tab-confounds:
...
Across all ten audit parts described above (bootstrap and LOOSO ranges shown together in @fig-bootstrap), the GG $>$ CC Frobenius effect remained robust. The finding cannot be attributed to sample composition, epoch count, data artifacts, task-label chance, or any of the seven tested peripheral-physiological/temporal-structure confounds (parts 7–8, 10; @tab-confounds). Part 9 — direct comparison against a conventional-EEG baseline — is reported separately below (§R5a) because it addresses a different question (whether MNJ is recoverable from simpler EEG summaries) rather than a confound for the Frobenius effect itself; part 10 — event/response-rate sensitivity — is reported in §R5b.
...
This work emerged through structured correspondence among human and AI collaborators; large language models from OpenAI, Anthropic, and Google assisted in literature synthesis, drafting, peer review, and editorial refinement. We also gratefully acknowledge the original investigators, clinical teams, participants, and data stewards who created, curated, and shared the public datasets and metadata that made these analyses possible.
...
Licensing, stewardship, and any related trademark notes are summarized at `https://noeticdiffusion.com/license.html` and do not alter the scientific claims or reproducibility boundaries discussed here.

= Data and Code Availability

Reference implementations and analysis pipelines:
`https://github.com/NoeticDiffusion/nmd-analysis`

Ingestion pipeline:
`https://github.com/NoeticDiffusion/NeuralManifoldDynamics-dataset`

Datasets and related public bundles:
`https://github.com/NoeticDiffusion/NeuralManifoldDynamics-dataset`
