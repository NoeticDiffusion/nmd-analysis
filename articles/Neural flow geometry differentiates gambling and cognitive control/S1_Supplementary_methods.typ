...
= Dataset and NMD measurement contract
...
= Epoch structure and MNPS/MNJ export
...
*How `mnps_3d` and `coords_9d` are computed from EEG.* The operational construction of the canonical 3D chart and the stratified 9D chart is now documented as a versioned, auditable measurement contract in a dedicated methods paper. @langell2026nmdm For the `ds004511` EEG reference configuration used throughout this article, `mnps_3d = [m, d, e]` is a fixed, release-bound weighted projection of nine EEG-derived subcoordinates $x_t^((9)) = [m_a, m_e, m_o, d_n, d_l, d_s, e_e, e_s, e_m]$:

$ m = 0.62 m_a + 0.55 m_e + 0.45 m_o, quad d = 0.50 d_n + 0.82 d_l + 0.28 d_s, quad e = 0.85 e_e + 0.62 e_s + 0.03 e_m $

with each subcoordinate resolved from a specific EEG feature under the active contract: $m_a = -0.5 dot "eeg_delta" - 0.5 dot "eeg_theta"$ (slow-wave power), $m_e = -"eeg_alpha"$ (alpha power, sign-flipped), $m_o = "eeg_beta_alpha"$ (beta/alpha ratio), $d_n = "eeg_gamma"$ (gamma power), $d_l = "eeg_hjorth_mobility"$, $d_s = "eeg_alpha_theta"$ (alpha/theta ratio), $e_e = "eeg_permutation_entropy"$, and $e_s = "eeg_hjorth_complexity"$. The ninth subcoordinate, $e_m$, is resolved per epoch through a nine-level empirical-fallback priority hierarchy (`ecg_rmssd` → `ecg_hr_bpm` → `ppg_rate_bpm` → `ppg_amplitude_mean` → `pupil_dilation_velocity` → `pupil_diameter_std` → `eog_blink_rate` → `meg_highfreq_power_30_45` → `eeg_highfreq_power_30_45`), taking the first finite source available and recording the chosen source in `embodied_arousal_proxy_source`; in the EEG-only `ds004511` reference path used here, this resolves to EOG blink rate or high-frequency EEG power for essentially all epochs. These projection weights and feature mappings are release-fixed operational priors chosen at contract-design time to preserve recomposability, modality-level measurability, and Jacobian-estimation stability — they are not fitted per-contrast, and are not claims that `(m, d, e)` uniquely and completely identify the theoretical NDT state. See @langell2026nmdm (§Axis Construction, §Rationale for the Stratified 9D Contract) for the full derivation, the fMRI-side analogue mapping, and the estimator-hygiene rationale behind this specific weighting.

*How the per-epoch Jacobian is estimated.* `jacobian_subject_anchored/J_hat` is a chart-level Jacobian, $J(x_t, t) = partial f(x, t) \/ partial x |_(x=x_t)$, estimated empirically from the local `mnps_3d` trajectory at ingest time rather than derived algebraically from a specified closed-form drift field; @langell2026nmdm (§B.4, §Jacobian Validity Domain) gives the formal definition and the coverage/finite-support/conditioning constraints under which estimation is considered valid, and documents Jacobian export as conditional on these constraints rather than guaranteed by name alone.

= Meta-Noetic Jacobian metrics
...
= EDA extraction (standalone script)
...
= Physiological confound and EAP feature definitions
...
= Robustness and confound audit protocol
